...
class EmailExtractorApp(QWidget):
    def __init__(self):
        ...
        self.label = QLabel("Select one or more files to extract email addresses.")
        self.layout.addWidget(self.label)

        self.button_load = QPushButton("Load Files")
    ...
    def load_files(self):
        file_names, _ = QFileDialog.getOpenFileNames(self, "Select Files", "", "Supported Files (*.txt *.pdf *.xls *.xlsx *.doc *.docx *.ppt *.pptx);;All Files (*)")
        content = ""
    ...
    def extract_emails(self, text):
        ...
        if emails:
            ...
            if output_directory:
                ...
                self.label.setText(f"{len(emails)} email addresses extracted and saved to '{output_file_name}'.")
            else:
                self.label.setText("No directory selected.")
        else:
            self.label.setText("No email addresses found.")
